...
class Bookmarks
{
    ...
    protected function openFile(string $fileName)
    {
        if (!file_exists($fileName)) {
            throw new BookmarksException("Plik CSV nie istnieje: $fileName");
        }

        $handle = fopen($fileName, 'r');
        if (!$handle) {
            throw new BookmarksException("Nie udało się otworzyć pliku: $fileName");
        }
    ...
    {
        if (empty($headers)) {
            throw new BookmarksException("Brak nagłówków w pliku CSV.");
        }
